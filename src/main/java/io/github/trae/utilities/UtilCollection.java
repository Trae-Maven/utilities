...
public class UtilCollection {
    ...
    public static <Type> Optional<Type> search(final Collection<? extends Type> collection, final Predicate<Type> equalsPredicate, final Predicate<Type> containsPredicate, final Consumer<List<Type>> listConsumer, final Consumer<String> messageConsumer, final Function<Type, String> resultFunction, final String input, final boolean inform) {
        ...
        if (inform) {
            String message = "No matches found [<yellow>%s</yellow>].".formatted(input);
            ...
            if (!(list.isEmpty())) {
                ...
                message = "<yellow>%s</yellow> matches found [%s]".formatted(list.size(), String.join("<gray>, </gray>", results));
            }
